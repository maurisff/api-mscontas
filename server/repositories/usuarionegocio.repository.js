...
const Model = mongoose.model('UsuarioNegocio');
...
exports.negociosDoUsuario = async (usuarioId) => Model.find({ usuarioId })
  .select('negocioId, ativo -_id')
  .populate({
    path: 'negocioId',
    select: 'nome ativo _id',
    options: { path: 'negocio' },
  }).exec();
